...
const AboutCard = ({ id, open, imgUrl, title, text, index, handleClick }) => {
  return (
    ...
    >
      ...
      {!open ? (
        <div className="absolute z-0 bg-secondary self-end py-8 lg:bottom-0 w-[100%] h-[40%] flex justify-center items-center rounded-b-xl">
          <h3 className="font-raleway font-[400] text-[16px] md:text-[24px] text-darkGrey lg:rotate-[-90deg] lg:origin-[0,0] ">
            {title}
          </h3>
        </div>
      ) : (
        <div className="absolute bottom-0 py-8 px-8 justify-start w-full flex-col bg-secondary min-h-max lg:h-[40%] lg:overflow-hidden rounded-b-xl transition-all ease-in-out">
          <h3 className="font-raleway font-[600] text-[20px] md:text-[24px] text-violet ">
            {title}
          </h3>
